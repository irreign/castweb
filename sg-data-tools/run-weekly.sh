...
HDB_DATASET_ID=""     # fill in once you find "Resale Flat Prices" on data.gov.sg
...
if [ -n "$HDB_DATASET_ID" ]; then
  echo "[$DATE] Fetching HDB resale..." >> "$LOG"
  node fetch-hdb-resale.mjs "$HDB_DATASET_ID" $HDB_TOWN --months=1 >> "$LOG" 2>&1
  ...
  echo "[$DATE] Skipping HDB resale — HDB_DATASET_ID not set yet, see the comment near the top of this file." >> "$LOG"
fi
